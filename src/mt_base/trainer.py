"""Generic seq2seq training loop.

Works with any encoder-decoder ``nn.Module`` that exposes:

* ``forward(src, tgt_in, src_pad_mask=...) -> logits``
* ``cfg`` attribute (a dataclass-like object with at least ``vocab_size``)
* ``state_dict()`` / ``load_state_dict()``

Both :class:`bi_mamba_mt.model.BiMambaTranslator` and
:class:`transformer_mt.model.TransformerTranslator` satisfy this contract.
"""
