...
* Encoder = stack of ``EncoderLayer`` re-using
  :class:`bi_mamba_mt.modules.bi_mamba.BiMambaBlock` (forward + reversed
  Mamba scan over the source). No source positional encoding because the
  Bi-Mamba scan/conv already injects positional information.
* Decoder = ``nn.TransformerDecoder`` (masked self-attention →
  cross-attention → FFN, pre-norm, GELU). Sinusoidal positional encoding
  is added to the target embedding because decoder self-attention is
  position-agnostic without it.
* Tied input / output embedding (``lm_head.weight = embedding.weight``).

Init policy
~~~~~~~~~~~
We deliberately do NOT call a global ``self.apply(_init_weights)`` over
the whole module tree. Mamba blocks rely on a custom initialisation of
``dt_proj.bias`` (softplus-inverse of a uniform sample), ``A_log``
(``log(arange(1, d_state+1))``), and ``D`` (ones), all set up inside
:class:`bi_mamba_mt.modules.mamba_block.MambaBlock`. A naive global init
would re-randomise these tensors and silently break the SSM dynamics.

Instead we precompute the set of modules belonging to any
:class:`MambaBlock` subtree and skip them during init. Non-Mamba modules
(target positional encoding has no learnable params, embedding, encoder
FFN, ``BiMambaBlock.combine``, the entire Transformer decoder, lm_head)
are initialised explicitly:

* ``nn.Embedding`` ⇒ ``normal(0, 0.02)``, pad row zeroed.
* ``nn.Linear``    ⇒ ``xavier_uniform`` weight, zero bias (skipped if
  the parameter is flagged ``_no_reinit``).
* ``nn.LayerNorm`` ⇒ left at PyTorch defaults (weight 1, bias 0).

A unit test (``tests/test_hybrid.py``) asserts that
``MambaBlock.A_log``, ``D`` and ``dt_proj.bias`` are preserved after
:class:`HybridMambaAttentionTranslator` construction.

The default config (``d_model=384``, 5+5 layers, encoder FFN 960, decoder
FFN 1536, vocab 16k) yields ~32.7M parameters — comparable to
:class:`bi_mamba_mt.model.BiMambaTranslator` (32.4M) and
:class:`transformer_mt.model.TransformerTranslator` (30.8M), so any BLEU
gap reflects architectural differences.
"""
...
from bi_mamba_mt.modules.bi_mamba import BiMambaBlock
...
class HybridEncoderLayer(nn.Module):
    """Bi-Mamba encoder layer with FFN. Same shape as
    :class:`bi_mamba_mt.model.EncoderLayer` but with a configurable FFN
    hidden size so the hybrid stays in the 32–36M parameter target.
    """
    ...
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        self.ffn_w1 = nn.Linear(cfg.d_model, cfg.encoder_d_ff)
        self.ffn_w2 = nn.Linear(cfg.encoder_d_ff, cfg.d_model)
...
class HybridMambaAttentionTranslator(nn.Module):
    """Bi-Mamba encoder + Transformer decoder.

    Public API is identical to
    :class:`bi_mamba_mt.model.BiMambaTranslator` and
    ...
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        # Source: NO positional encoding (Bi-Mamba scan/conv injects order).
        # Target: sinusoidal PE (decoder self-attention needs explicit positions).
        self.tgt_pos_enc = SinusoidalPositionalEncoding(
            cfg.d_model, max_len=cfg.max_tgt_len + 8
        )
        self.dropout = nn.Dropout(cfg.dropout)

        # ---- Encoder: Bi-Mamba stack ---------------------------------
        self.encoder_layers = nn.ModuleList(
            [HybridEncoderLayer(cfg) for _ in range(cfg.n_encoder_layers)]
        )
        ...
        decoder_layer = nn.TransformerDecoderLayer(
            d_model=cfg.d_model,
            nhead=cfg.n_decoder_heads,
            dim_feedforward=cfg.decoder_d_ff,
            dropout=cfg.dropout,
            activation="gelu",
            batch_first=True,
            norm_first=True,
        ...
        # IMPORTANT: scoped initialisation that skips MambaBlock subtrees.
        self._init_non_mamba_weights()

    # ------------------------------------------------------------------
    def _init_non_mamba_weights(self) -> None:
        """Initialise every module that is not part of a MambaBlock.

        Mamba blocks have custom initialisation for ``dt_proj.bias``,
        ``A_log`` and ``D``; a global ``self.apply(...)`` would silently
        re-randomise those tensors. We collect the ids of all modules
        inside any MambaBlock subtree and skip them.
        """
        ...
        for module in self.modules():
            ...
            # nn.LayerNorm uses PyTorch's default (ones / zeros) which is fine.
    ...
    def _causal_mask(length: int, device: torch.device) -> torch.Tensor:
        # nn.Transformer uses True at positions to mask out (additive -inf).
        return torch.triu(
    ...
    def decode(
        ...
        tgt_pad_mask = tgt.eq(self.cfg.pad_id)
        x = self.decoder(
            x,
            encoder,
            tgt_mask=causal,
    ...
    def _beam_decode(
        ...
        We re-feed the full target prefix to the decoder at every step.
        That is O(T^2) per example but adequate for evaluation with
        ``beam_size`` ≈ 4.
        """
        device = src.device
